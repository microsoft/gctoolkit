...
public class GCToolKit {
    ...
    /// `-Dgctoolkit.debug=all,-com.microsoft.gctoolkit.parser.UnifiedG1GCParser`
    ///
    /// @param message Supplies the message to log. If null, nothing will be logged.
    public static void LOG_DEBUG_MESSAGE(@Nullable Supplier<String> message) {
        if (DEBUGGING && message != null) {
            StackTraceElement[] stackTrace = Thread.currentThread().getStackTrace();
            String methodName = stackTrace[2].getMethodName();
            String className = stackTrace[2].getClassName();
            String fileName = stackTrace[2].getFileName();
            var lineNumber = stackTrace[2].getLineNumber();
            if (isDebugging(className)) {
                IO.println("DEBUG: %s.%s(%s:%d): %s".formatted(className, methodName, fileName, lineNumber, message.get()));
            }
        }
    }

    private final Set<DataSourceParser> registeredDataSourceParsers = new HashSet<>();
    ...
    /// provides the [Aggregation] API, define the following
    /// in `module-info.java`:
    /// ```
    /// import com.microsoft.gctoolkit.aggregator.Aggregation;
    ...
    /// Registers an `Aggregation` class which can be used to perform analysis
    /// on `JVMEvent`s. GCToolKit will instantiate the Aggregation when needed.
    ///
    /// The [JavaVirtualMachine#getAggregation(Class)]
    /// API will return an Aggregation that was used in the log analysis. Even though
    /// an Aggregation was registered, the `getAggregation` method will return
    ...
    private void loadDataSourceChannel() {
        if ( dataSourceChannel == null) {
            var serviceLoader = ServiceLoader.load(DataSourceChannel.class);
    ...
    private void loadJVMEventChannel() {
        if ( jvmEventChannel == null) {
            var serviceLoader = ServiceLoader.load(JVMEventChannel.class);
            if (serviceLoader.findFirst().isPresent()) {
    ...
    /// This method should be called before the [#analyze(DataSource)] method.
    /// DataSourceParsers loaded by this method are used in place of those that are ordinarily loaded via
    /// the service provider interface.
    /// Use the [#addDataSourceParser(DataSourceParser)] method to load a DataSourceParser in addition
    ...
    private Set<EventSource> loadDataSourceParsers(Diary diary) {
        ...
        if (dataSourceParsers.isEmpty()) {
            ...
            dataSourceParsers = Arrays.stream(parsers)
                    .map(parserName -> {
                        try {
                            var clazz = forName(parserName, true, Thread.currentThread().getContextClassLoader());
    ...
    /// that were [registered][#loadAggregation(Aggregation)], if appropriate for
    /// the GC log file.
    ///
    ...
    public JavaVirtualMachine analyze(DataSource<?> dataSource) throws IOException  {
        var logFile = (GCLogFile)dataSource;
        var events = loadDataSourceParsers(logFile.diary());
        JavaVirtualMachine javaVirtualMachine = loadJavaVirtualMachine(logFile);
        try {
            var filteredAggregators = filterAggregations(events);
    ...
    private List<Aggregator<? extends Aggregation>> filterAggregations(Set<EventSource> events) {
        ...
        for (Aggregation aggregation : registeredAggregations) {
            ...
            var constructor = constructor(aggregation);
            if (constructor == null) {
