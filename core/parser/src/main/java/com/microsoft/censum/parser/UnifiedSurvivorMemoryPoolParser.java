...
import com.microsoft.censum.event.jvm.SurvivorRecord;
import com.microsoft.censum.parser.jvm.Decorators;
import com.microsoft.censum.parser.jvm.LoggingDiary;

import static com.microsoft.censum.parser.unified.UnifiedPatterns.CPU_BREAKOUT;
import static com.microsoft.censum.parser.unified.UnifiedPatterns.JVM_EXIT;
...
public class UnifiedSurvivorMemoryPoolParser extends UnifiedGCLogParser implements TenuredPatterns {
     ...
     */
    GCParseRule DESIRED_SURVIVOR_SIZE = new GCParseRule("DESIRED_SURVIVOR_SIZE", "Desired survivor size " + COUNTER + " bytes, new threshold " + COUNTER + " \\(max threshold " + COUNTER + "\\)");
    GCParseRule AGE_TABLE_HEADER = new GCParseRule("AGE_TABLE_HEADER", "Age table with threshold " + COUNTER + " \\(max threshold " + COUNTER + "\\)");
    GCParseRule AGE_RECORD = new GCParseRule("AGE_RECORD", "- age\\s+" + COUNTER + ":\\s+" + COUNTER + " bytes,\\s+" + COUNTER + " total");
